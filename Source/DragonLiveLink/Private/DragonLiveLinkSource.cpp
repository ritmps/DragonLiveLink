...
#define LOCTEXT_NAMESPACE "DragonLiveLinkSource"
...
FDragonLiveLinkSource::FDragonLiveLinkSource(FIPv4Endpoint InEndpoint)
: Socket(nullptr)
, Stopping(false)
, Thread(nullptr)
, isRunning(false)
...
FDragonLiveLinkSource::~FDragonLiveLinkSource()
{
	Stop();
	if (Thread != nullptr)
	{
...
void FDragonLiveLinkSource::ReceiveClient(ILiveLinkClient* InClient, FGuid InSourceGuid)
{
	Client = InClient;
...
bool FDragonLiveLinkSource::IsSourceStillValid() const
{
...
bool FDragonLiveLinkSource::RequestSourceShutdown()
{
	Stop();
...
uint32 FDragonLiveLinkSource::Run()
{
...
void  FDragonLiveLinkSource::SetupSubjects(const FString JsonString, TArray<Subject> &SubjectList)
{
...
void FDragonLiveLinkSource::SendFrameToLiveLink(const TArray<Subject> SubjectList, const TArray<float> FrameValueList)
{
...
void FDragonLiveLinkSource::HandleReceivedData(TSharedPtr<TArray<uint8>, ESPMode::ThreadSafe> ReceivedData)
{
    if (Stopping) {
